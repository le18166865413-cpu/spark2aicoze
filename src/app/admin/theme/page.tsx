  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-card border border-border rounded-xl p-4 space-y-3">
            ...
            <div className="grid grid-cols-5 sm:grid-cols-8 md:grid-cols-11 lg:grid-cols-9 gap-1">
              {THEMES.map((t) => (
                <button
                  key={t.id}
                  onClick={() => handleThemeChange(t.id)}
                  className={`flex flex-col items-center gap-1 p-1.5 rounded-lg border cursor-pointer transition-colors ${theme === t.id && !isCustom ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/50'}`}
                >
                  <div
                    className="w-5 h-5 rounded flex items-center justify-center shrink-0"
                    style={{ backgroundColor: t.hex }}
                  >
                    {theme === t.id && !isCustom && <Check className="w-3 h-3 text-white drop-shadow" />}
                  </div>
                  <span className="text-[9px] text-muted-foreground truncate w-full text-center leading-tight">{t.name}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="bg-card border border-border rounded-xl p-4 space-y-3">
            <div className="flex items-center gap-2">
              ...
              {isCustom && (
                <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full bg-primary/10 text-primary text-[10px]">
            ...
            <div className="space-y-2">
              ...
              {/* Color grids */}
              {[
                { label: '红色系', colors: ['#FFF0F0','#FFE0E0','#FFCCCC','#FFB3B3','#FF9999','#FF8080','#FF6666','#FF4D4D','#FF3333','#FF1A1A','#E60000','#CC0000','#B30000','#990000','#800000','#660000'] },
                { label: '橙色系', colors: ['#FFF5EB','#FFE8D0','#FFD6A8','#FFC280','#FFB366','#FF9E3D','#FF8C1A','#FF7700','#E66B00','#CC5F00','#B35300','#994700','#803B00','#662F00','#4D2400','#331A00'] },
                { label: '黄色系', colors: ['#FFFFF0','#FFFFE0','#FFFDE5','#FFF9B3','#FFF580','#FFF14D','#FFED1A','#FFE600','#E6D000','#CCBA00','#B3A400','#998E00','#807800','#666200','#4D4C00','#333600'] },
                { label: '绿色系', colors: ['#F0FFF0','#DFFFDF','#CCFFCC','#B3FFB3','#99FF99','#80FF80','#66FF66','#4DFF4D','#33FF33','#1AFF1A','#00E600','#00CC00','#00B300','#009900','#008000','#006600'] },
                { label: '青色系', colors: ['#F0FFFF','#DFFFFF','#CCFFFF','#B3FFFF','#99FFFF','#80FFFF','#66FFFF','#4DFFFF','#33FFFF','#1AFFFF','#00E6E6','#00CCCC','#00B3B3','#009999','#008080','#006666'] },
                { label: '蓝色系', colors: ['#F0F5FF','#D9E8FF','#B3D4FF','#8CBCFF','#66A3FF','#4D8FFF','#3379FF','#1A63FF','#004DE6','#003ACC','#002DB3','#002199','#001A80','#001266','#000D4D','#000833'] },
                { label: '紫色系', colors: ['#F5F0FF','#E6D9FF','#D4B3FF','#C299FF','#B080FF','#9C66FF','#884DFF','#7433FF','#601AFF','#5200E6','#4700CC','#3C00B3','#310099','#260080','#1C0066','#11004D'] },
                { label: '粉色系', colors: ['#FFF0F5','#FFD9E8','#FFB3D4','#FF99C4','#FF80B4','#FF66A3','#FF4D93','#FF3383','#FF1A73','#E60066','#CC0059','#B3004D','#990040','#800033','#660026','#4D001A'] },
                { label: '中性色', colors: ['#FFFFFF','#FAFAFA','#F5F5F5','#EEEEEE','#E5E5E5','#D4D4D4','#BFBFBF','#A3A3A3','#8C8C8C','#737373','#595959','#404040','#2E2E2E','#262626','#171717','#0A0A0A'], lightColors: ['#FFFFFF','#FAFAFA','#F5F5F5','#EEEEEE'] },
              ].map(({ label, colors, lightColors }) => (
                <div key={label} className="space-y-0.5">
                  <span className="text-[9px] font-medium text-muted-foreground uppercase tracking-wider">{label}</span>
                  <div className="flex flex-wrap gap-[1px]">
                    {colors.map(hex => (
                      <button key={hex} onClick={() => handleCustomHex(hex)} className={`w-[18px] h-[18px] rounded-sm cursor-pointer border-0 transition-transform hover:scale-125 ${isCustom && customHex.toLowerCase() === hex.toLowerCase() ? 'ring-1 ring-primary scale-125' : ''} ${lightColors?.includes(hex) ? 'border border-border/30' : ''}`} style={{ backgroundColor: hex }} title={hex} />
                    ))}
                  </div>
                </div>
              ))}
              ...
              <div className="flex items-center gap-2 pt-1.5 border-t border-border/50">
                ...
                />
                <div
                  className="w-8 h-8 rounded-lg border border-border shadow-inner shrink-0"
                  style={{ backgroundColor: currentHex }}
                />
                <span className="text-xs font-mono text-foreground">{currentHex.toUpperCase()}</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-card border border-border rounded-xl p-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              {MODES.map((m) => (
                <button key={m.id} onClick={() => handleModeChange(m.id)} className={`p-2.5 rounded-lg border cursor-pointer transition-colors text-center ${mode === m.id ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/50'}`}>
            ...
            {effects.metallicTextEnabled && (
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div className="space-y-1">
                    <label className="text-xs font-medium text-muted-foreground">中间高光</label>
                ...
                <div
                  className="h-8 rounded-lg flex items-center justify-center text-base font-bold border border-border"
                  style={{
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between py-1.5 border-b border-border/50">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">主色按钮的柔和发光</div>
                </div>
              ...
              <div className="flex items-center justify-between py-1.5">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">卡片边框渐变发光</div>
                </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between py-1.5 border-b border-border/50">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">顶部主题色渐变过渡</div>
                </div>
              ...
              <div className="flex items-center justify-between py-1.5">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">导航栏主题色渐变背景</div>
                </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-xs text-muted-foreground">选择主题后立即预览效果，点击保存按钮永久保存。</span>
      </div>
    </div>
  );
}
