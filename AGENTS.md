...
- **数据存储**: JSON 文件 (data/gallery.json)
...
- 使用 JSON 文件 (`data/gallery.json`) 存储图片记录
- 图片文件存储在 S3 兼容对象存储中 (coze-coding-dev-sdk S3Storage)
- 图片 URL 通过 `generatePresignedUrl` 生成签名访问链接
- 永久 URL 通过 `/sign-url` 接口生成 (expire_time: 0)
- GalleryImage 结构：id, imageKey, prompt, width, height, views, downloads, type, taskId, createdAt
