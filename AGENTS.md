...
- **邮件发送**: Nodemailer (SMTP，支持 QQ/163 等邮箱，配置存储在 admin_settings)
...
- gallery_images 表结构：id, prompt, url, image_key, width, height, views, downloads, model, ratio, task_id, user_id, creator_name, created_at
- users 表结构：id, username, password, email, nickname, role, status, created_at, updated_at
- user_sessions 表结构：id, user_id, expires_at, created_at
- email_verification_codes 表结构：id, email, code, used, attempts, expires_at, created_at
- 用户注册默认 status=pending，需管理员审批后才能登录（approved/rejected）
- 邮箱验证码登录：验证码 5 分钟有效，同一邮箱 60 秒内只能发送 1 次，同一验证码最多尝试 5 次
- Supabase 客户端：`src/storage/database/supabase-client.ts`

## GrsAI API

- 图片生成：`POST https://grsai.dakka.com.cn/v1/api/generate` (SSE 流式，统一端点)
- 任务结果查询：`POST https://grsai.dakka.com.cn/v1/api/result`
- 模型：image2-vip / image2 / nano-banana-fast / nano-banana-2 / nano-banana-pro-vip
...
| `/api/config` | GET | 获取公共配置（站点信息、模板、模型、比例等） |
| `/api/auth/register` | POST | 用户注册（默认 pending，需管理员审批） |
| `/api/auth/login` | POST | 用户登录（邮箱+验证码 或 用户名+密码） |
| `/api/auth/logout` | DELETE | 用户登出 |
| `/api/auth/me` | GET | 获取当前用户信息 |
| `/api/auth/send-code` | POST | 发送邮箱验证码 |
| `/api/admin/users` | GET | 获取用户列表（需管理员 Cookie 认证） |
| `/api/admin/users` | POST | 创建用户（管理员创建，自动 approved） |
| `/api/admin/users` | PUT | 更新用户（审批/拒绝/编辑/重置密码） |
| `/api/admin/users` | DELETE | 删除用户 |
| `/api/admin/settings` | GET | 获取所有设置（需认证） |
| `/api/admin/settings` | PUT | 保存设置（需认证） |
| `/api/admin/test-smtp` | GET | 测试 SMTP 连接（需认证） |
...
- 访问路径：`/admin`
- 登录页面：`/admin/login`
- 管理员用户名：`admin`，密码：`666666`
- 认证方式：Cookie (user_session)，有效期 7 天
- 仅 role=admin 的用户可以访问后台页面，非管理员自动重定向
- 数据库表：admin_settings (设置)、users (用户)、user_sessions (会话)
- 所有管理 API 需通过 Cookie 认证并验证 admin 角色
